...
class NetworkTools:
    """
    OPTIMIZED: Only fetches network data when absolutely necessary
    Skips network calls for casual/conversational queries
    """
    ...
    def __init__(self):
        self.network_keywords = [
            ...
            'performance', 'analytics', 'metrics', 'stats', 'vnode'
        ]
        self.pubkey_pattern = re.compile(r'\b[1-9A-HJ-NP-Za-km-z]{43,44}\b')
    
    def should_fetch_network_data(self, query: str, query_type: str = None) -> bool:
        """
        OPTIMIZED: Skip network fetch for casual/conversational queries
        """
        # Never fetch for casual queries
        if query_type == "casual":
            return False
        
        # Never fetch for conversational queries (they use history)
        if query_type == "conversational":
            return False
        
        # For factual/technical, only fetch if network keywords present
    ...
    async def fetch_relevant_data(self, query: str, query_type: str = None) -> Dict:
        """
        OPTIMIZED: Main method with early exit for non-network queries
        ...
        # EARLY EXIT: Skip network fetch if not needed
        if not self.should_fetch_network_data(query, query_type):
            return result
